...
\begin{titlepage}
    ...
    \begin{tikzpicture}
        \node[draw=primaryblue, line width=2pt, rounded corners=10pt, 
              ...
              fill=primaryblue!10] {
            \textcolor{primaryblue}{\LARGE\bfseries SaaS Monitor}
        };
    \end{tikzpicture}
...
\begin{tikzpicture}[
    ...
    box/.style={rectangle, draw=primaryblue, fill=primaryblue!10, 
                ...
                text centered, rounded corners=3pt, font=\scriptsize},
    arrow/.style={->, >=stealth, thick, primaryblue}
]

\node[box] (browser) {Browser};
\node[box, below=of browser] (flask) {Flask (5000)};
\node[box, below left=1cm and 0.5cm of flask] (logstash) {Logstash};
\node[box, below=of flask] (es) {Elasticsearch};
\node[box, below right=1cm and 0.5cm of flask] (kibana) {Kibana};
\node[box, left=1.5cm of es] (mongodb) {MongoDB};
\node[box, right=1.5cm of es] (redis) {Redis};
\node[box, below=of es] (prometheus) {Prometheus};

\draw[arrow] (browser) -- (flask);
\draw[arrow] (flask) -- (es);
\draw[arrow] (flask) -- (mongodb);
\draw[arrow] (flask) -- (redis);
\draw[arrow] (logstash) -- (es);
\draw[arrow] (kibana) -- (es);
