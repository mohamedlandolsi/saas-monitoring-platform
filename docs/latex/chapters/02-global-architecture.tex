...
% Presentation layer
\node[box, below=of browser] (flask) {Flask Web App};
\node[box, right=2cm of flask] (socketio) {Socket.IO};

% ELK Stack
\node[box, below left=2cm and 1cm of flask] (logstash) {Logstash};
\node[box, below=of flask] (elasticsearch) {Elasticsearch};
\node[box, below right=2cm and 1cm of flask] (kibana) {Kibana};

% Data stores
\node[db, below left=1.5cm and 0cm of logstash] (mongodb) {MongoDB};
\node[db, below right=1.5cm and 0cm of kibana] (redis) {Redis};

% Observability
\node[box, below=of elasticsearch] (prometheus) {Prometheus};
\node[box, right=1.5cm of prometheus] (grafana) {Grafana};
\node[box, left=1.5cm of prometheus] (jaeger) {Jaeger};
